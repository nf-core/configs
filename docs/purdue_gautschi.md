...
Gautschi is an AMD EPYC 9654 (Genoa) cluster with 192 cores and 384 GB RAM per standard CPU node, plus NVIDIA L40 and H100 GPU partitions. See the [RCAC Gautschi user guide](https://www.rcac.purdue.edu/compute/gautschi) for hardware and policy details.
...
```bash
slist                     # list your Gautschi accounts
...
The profile sets `--gres=gpu:N` from each task's `accelerator` directive. `ai` nodes are scarce; use `smallgpu` unless your workflow specifically needs H100 throughput, NVLink, or > 48 GB GPU memory.
...
`standby` does not apply to `highmem` or GPU jobs.
...
```bash
export NXF_SINGULARITY_CACHEDIR=$RCAC_SCRATCH/.apptainer/cache    # default
