...
To use, run the pipeline with `-profile eb-codon`. This will download and launch the [`ebi-codon.config`](../conf/ebi-codon.config) which has been pre-configured with a setup suitable for the codon cluster.
